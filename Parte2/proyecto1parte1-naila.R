#Se está compartiendo la base de datos con información recopilada durante 2011 en la ciudad de New York de un programa conocido 
#como "Stop and Risk" al igual que un único archivo pdf con el significado de las columnas.
#El proyecto consiste en entregar un documento pdf con la siguiente información después de haber realizado una revisión: 
...
sum(base$frisked, na.rm = T) #número de personas cacheadas
381704/685724
...
sum(base$contrabn, na.rm = T) #número de personas a las que se les encontró artículos de contrabando
...
sum(base$pistol, na.rm = T) #número de personas a las que se les encontró una pistola
...
sum(base$arstmade, na.rm = T) #número de personas a las que arrestaron
